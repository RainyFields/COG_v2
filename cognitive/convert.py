...
class TaskInfoConvert(object):
    ...
    def __init__(self, task_example=None, task=None, objset=None):
        ...
        if task is None or objset is None:
            ...
            cur_task = self.task_info[0]
            self.frame_info = fi.FrameInfo(task=task, objset=objset)
        ...
        for i, old_frame, new_frame in enumerate(zip(self.frame_info[start:], new_task_info.frame_info)):
            ...
            if not old_frame.objs or not new_frame.objs:
                ...
                if np.random.random() < reuse:  # use previous frame info and reinit new task
                    ...
                    targets = new_task_info.task.get_target(self.frame_info.objset)
