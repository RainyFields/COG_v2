...
class TaskInfoConvert(object):
    ...
    def __init__(self, task_example=None, task=None, objset=None):
        ...
        if task is None or objset is None:
            ...
            self.task_info[0]["is_intact"] = task.is_intact
            self.task_info[0]["question"] = str(task)
            self.task_info[0]["answers"] = [get_target_value(t) for t in task.get_target()]
            self.task_info[0]["task_len"] = task.n_frames

            self.n_epochs = self.task_info[0]["task_len"]
            ...
            for i in range(self.n_epochs):
                ...
                # iterate over task_example["objects"] and check epochs to determine whether that eq current i
                self.frame_info[i]["objs"] = []
                # TODO: move this objset loop outside of frame loop
                objs_count = 0
                for obj in objset:
                    if obj.epoch[0] != i:
                        break
                    self.frame_info[i]["objs"].append({})
                    for features in ["location", "shape", "color", "is_distractor"]:
                        self.frame_info[i]["objs"][objs_count][features] = obj.getattr(features)
                    objs_count += 1
                ...
                self.frame_info[i]["description"] = []
                if i == self.n_epochs - 1:
                    self.frame_info[i]["action"] = self.task_info[0]['answers']
                    self.frame_info[i]["description"].append(["ending of task %d" % 0])
                else:
                    self.frame_info[i]["action"] = None
                    if i == 0:
    ...
    def merge(self, new_task_info):
        ...
        self.add_frames()
        lastk = new_task_info.n_epochs
        max_when = len(self.frame_info)
        start_idx = lastk
        ...
        while not start:
            # queue structure
            if all(frame_info.is_intact for frame_info in self.frame_info[-start_idx - 1::]):

            else:
                if all(frame_info.shareable for frame_info in self.frame_info[]):

        return

    def add_frames(self, frame_info_list=None):
